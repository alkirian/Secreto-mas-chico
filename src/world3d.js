...
this.veilMaterial=new THREE.ShaderMaterial({transparent:true,depthTest:false,depthWrite:false,uniforms:{amount:{value:1},center:{value:new THREE.Vector2(.73,.48)}},vertexShader:'varying vec2 uvScreen;void main(){uvScreen=uv;gl_Position=vec4(position.xy,0.,1.);}',fragmentShader:'uniform float amount;uniform vec2 center;varying vec2 uvScreen;void main(){vec2 delta=(uvScreen-center)*vec2(1.78,1.0);float halo=exp(-dot(delta,delta)*10.0);float edge=exp(-dot(delta,delta)*1.65);vec3 light=vec3(.16,.11,.045)*halo+vec3(.018,.022,.025)*edge;gl_FragColor=vec4(vec3(.004,.009,.016)+light,amount);}' });
...
this.hero.position.set(p.x+19,HEIGHT-p.y-62,25);this.hero.scale.set(1+p.squash,(1-p.squash)*(1+Math.sin(t*2.2)*.008*blend),1);this.hero.visible=!(p.invincible>0&&Math.sin(t*24)<-.5);const stride=Math.sin(t*17)*Math.min(Math.abs(p.vx)/350,1);this.legs.forEach((m,i)=>{m.rotation.z=(p.ground?stride*.35: .22)*(i?1:-1);});this.arms.forEach((m,i)=>{m.position.y=p.rope?61:34;m.rotation.z=p.rope?(i?-0.25:.25):(i?1:-1)*stride*.22;m.scale.y=p.rope?1.55:1;this.hands[i].position.y=p.rope?82:19;});this.scarf.position.x=-29;this.scarf.rotation.z=.15+Math.sin(t*9)*.12;this.heroEyes.forEach((e,i)=>e.position.x=(blend>.3?Math.sign(o.x-p.x):p.face)*7+(i?-8:3));
...
this.renderer.autoClear=false;this.veilMaterial.uniforms.amount.value=ease;
