...
let src=fs.readFileSync('dist/game.js','utf8').replace('})();','window.test={leaveLetterQuest,update,start,reset,render,stepPhysics,respawn,get p(){return player},get plats(){return platforms},get walls(){return walls},get ropes(){return ropes},get enemies(){return enemies},get letters(){return letters},get state(){return {mode,gate,switchOn,bridge,finalBridge,flags,checkpoint,queue,dialog,cinema,orb,lettersComplete,questActive,doorTime,doorPassed}},unlockLetters(){letters.forEach(item=>item.collected=true);lettersComplete=true;},setPos(x,y){Object.assign(player,{x,y,vx:0,vy:0,ground:false,on:null,wall:0,wallLock:0,rope:null,airJump:true,invincible:0});buffer=coyote=0;},clearTalk(){queue=[];dialog=null;}};})();');vm.runInNewContext(src,sandbox);
...
for(const w of g.walls){g.setPos(w.x-38,w.y+70);g.p.vy=500;tick(1,{axis:1},true);assert.equal(g.p.wall,1);assert(g.p.vy<=120);const y=g.p.y;tick(12,{axis:1,jump:true,jp:true},true);assert(g.p.x<w.x-60,'wall launch pushes away');assert(g.p.y<y-50);assert(g.p.airJump,'wall jump restores aerial jump');}
