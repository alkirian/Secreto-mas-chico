...
rect(13420,540,250,360);rect(13830,650,1450,400);
...
const oldX=p.x,oldY=p.y;p.x=clamp(p.x+p.vx*dt,20,15050);p.wall=0;
...
if(!gate&&p.x+38>3745&&p.x<3830)p.x=3707;if(!switchOn&&p.x+38>5520&&p.x<5580)p.x=5482;
...
if(p.y>1020){respawn();event('fall',true,()=>say(['Estoy acá. ¡Otra vez!']));}
}
...
function reset(){level();player={x:180,y:660,vx:0,vy:0,w:38,h:62,ground:false,face:1,squash:0};orb={x:340,y:560,vx:0,vy:0};flags={};queue=[];dialog=null;gate=switchOn=bridge=finalBridge=false;ending=0;cam=0;checkpoint={x:180,y:660};particles=[];buffer=coyote=0;selection=1;hintTime=0;hint='';start();}
...
stepPhysics(dt,I,locked);
player.squash*=Math.exp(-12*dt);shake*=Math.exp(-8*dt);
event('path',player.x>550,()=>{say(['Por acá.']);showHint('SALTO dos veces · Doble salto');});
event('cp1',player.x>1050&&player.x<1350&&player.ground,()=>{checkpoint={x:1050,y:620};});
event('cp2',player.x>1950&&player.x<2350&&player.ground,()=>{checkpoint={x:1950,y:610};});
event('stompHint',player.x>930,()=>showHint('Saltá sobre los bichitos para deshacerlos.')); event('hard',player.x>1020,()=>say(['Ese parece difícil.']));event('well',player.x>1570,()=>{say(['Ah… sí que sabés.']);burst(orb.x,orb.y);});
...
cam=lerp(cam,clamp(player.x-570+player.vx*.35,0,13700),1-Math.exp(-4*dt));
...
window.gameStatus=()=>({doubleJump:player.airJump,wall:player.wall,rope:!!player.rope,enemies:enemies.filter(e=>e.alive).length,mode,x:Math.round(player.x),y:Math.round(player.y),gate,switchOn,bridge,finalBridge,flags:{...flags},platforms:platforms.length});
})();
