...
window.secretDisplay?.subscribe(quality=>{const scale=quality==='low'||quality==='2d'?.6:1;canvas.width=W*scale;canvas.height=H*scale;c.setTransform(scale,0,0,scale,0,0);});
...
// Dialogue text is visible only when explicitly associated with a recording.
...
addEventListener('keydown',e=>{if(e.target?.tagName==='SELECT')return;const code=keyCode(e);if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Space','Enter'].includes(code))e.preventDefault();if(code==='Escape'&&!e.repeat&&cinema.stage){e.preventDefault();skipCinema();return;}keys.add(code);if(code==='KeyF'&&!e.repeat)$('#full').click();});addEventListener('keyup',e=>keys.delete(keyCode(e)));addEventListener('blur',()=>{keys.clear();if(mode==='play')pause();});document.addEventListener('visibilitychange',()=>{if(document.hidden&&mode==='play')pause();});
...
function frame(ms){window.secretDisplay?.sample(ms-last);const elapsed=Math.min((ms-last)/1000,.05);last=ms;acc+=elapsed;const I=input();for(const k of ['jp','act','pp']){pendingInput[k] ||= I[k];I[k]=pendingInput[k];}let first=true;while(acc>=1/120){update(1/120,first?I:{...I,jp:false,act:false,pp:false});first=false;pendingInput={jp:false,act:false,pp:false};acc-=1/120;}render();requestAnimationFrame(frame);}requestAnimationFrame(frame);
